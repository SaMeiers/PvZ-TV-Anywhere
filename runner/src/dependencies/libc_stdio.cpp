...
constexpr std::uint32_t kUrandomToken     = 0xF1FFFF00;
constexpr std::uint32_t kProcMapsFileToken = 0xF1FFFF01;
constexpr std::uint32_t kProcCmdlineToken  = 0xF1FFFF02;

static std::string s_stdio_proc_maps;
static size_t s_stdio_maps_pos = 0;
static std::mutex s_stdio_maps_lock;

static const std::string s_stdio_cmdline = std::string("com.popcap.pvz\0", 15);
static size_t s_stdio_cmdline_pos = 0;
static std::mutex s_stdio_cmdline_lock;

static std::string generate_stdio_maps(const pvz2_elf_image_t *img) {
    std::string text;
    char line[256];
    if (img) {
...
void c_fopen(GuestCall &c) {
    ...
    if (gpath == "/proc/self/maps" || gpath.find("proc/self/maps") != std::string::npos) {
    ...
    bool is_read_only = (mode.find('w') == std::string::npos && 
    ...
    if (is_read_only) {
        if (!vfs::exists(c.rt, gpath, hpath)) {
            // Not logged: the engine probes several extensions/locales per resource.
...
void c_fclose(GuestCall &c) {
    if (c.arg(0) == kUrandomToken || c.arg(0) == kProcMapsFileToken || c.arg(0) == kProcCmdlineToken) {
        c.set_result(0);
        return;
    }
...
void c_fread(GuestCall &c) {
    ...
    if (c.arg(3) == kProcMapsFileToken) {
        std::lock_guard<std::mutex> lk(s_stdio_maps_lock);
        uint64_t total = (uint64_t)size * nmemb;
        if (s_stdio_maps_pos >= s_stdio_proc_maps.size() || total == 0) {
            c.set_result(0);
            return;
        }
        size_t avail = s_stdio_proc_maps.size() - s_stdio_maps_pos;
        size_t to_read = (total < avail) ? (size_t)total : avail;
        if (c.in_bounds(dst, (uint32_t)to_read)) {
            std::memcpy(&c.img->mem[dst], s_stdio_proc_maps.data() + s_stdio_maps_pos, to_read);
        }
        s_stdio_maps_pos += to_read;
        c.set_result((uint32_t)(to_read / size));
        return;
    }
    if (c.arg(3) == kProcCmdlineToken) {
        std::lock_guard<std::mutex> lk(s_stdio_cmdline_lock);
        uint64_t total = (uint64_t)size * nmemb;
        if (s_stdio_cmdline_pos >= s_stdio_cmdline.size() || total == 0) {
            c.set_result(0);
            return;
        }
        size_t avail = s_stdio_cmdline.size() - s_stdio_cmdline_pos;
        size_t to_read = (total < avail) ? (size_t)total : avail;
        if (c.in_bounds(dst, (uint32_t)to_read)) {
            std::memcpy(&c.img->mem[dst], s_stdio_cmdline.data() + s_stdio_cmdline_pos, to_read);
        }
        s_stdio_cmdline_pos += to_read;
        c.set_result((uint32_t)(to_read / size));
        return;
    }
    if (c.arg(3) == kUrandomToken) {
        std::uint64_t total = (std::uint64_t)size * nmemb;
        for (std::uint64_t i = 0; i < total; ++i) {
            c.write8(dst + (std::uint32_t)i, (std::uint8_t)(rand() & 0xFF));
        }
        c.set_result(nmemb);
        return;
    }
...
void c_feof(GuestCall &c) {
    if (c.arg(0) == kProcMapsFileToken) {
        std::lock_guard<std::mutex> lk(s_stdio_maps_lock);
        c.set_result(s_stdio_maps_pos >= s_stdio_proc_maps.size() ? 1u : 0u);
        return;
    }
    if (c.arg(0) == kProcCmdlineToken) {
        std::lock_guard<std::mutex> lk(s_stdio_cmdline_lock);
        c.set_result(s_stdio_cmdline_pos >= s_stdio_cmdline.size() ? 1u : 0u);
        return;
    }
    std::FILE *f = c.file(c.arg(0));
...
void c_fgets(GuestCall &c) {
    ...
    if (c.arg(2) == kProcMapsFileToken) {
        std::lock_guard<std::mutex> lk(s_stdio_maps_lock);
        if (s_stdio_maps_pos >= s_stdio_proc_maps.size() || dst == 0 || size <= 1) {
            c.set_result(0);
            return;
        }
        size_t next_nl = s_stdio_proc_maps.find('\n', s_stdio_maps_pos);
        size_t line_len = (next_nl != std::string::npos) ? (next_nl - s_stdio_maps_pos + 1) : (s_stdio_proc_maps.size() - s_stdio_maps_pos);
        if (line_len >= (size_t)size) line_len = size - 1;
        std::string line = s_stdio_proc_maps.substr(s_stdio_maps_pos, line_len);
        c.put_cstr(dst, line);
        s_stdio_maps_pos += line_len;
        c.set_result(dst);
        return;
    }
