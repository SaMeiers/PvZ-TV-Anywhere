...
void c_mutex_lock(GuestCall &c) {
    ...
    if (!gm->try_lock_for(kStuckWarnAfter, self)) {
        c.log("[stuck] tid=%u blocked >3s in pthread_mutex_lock(0x%08x) lr=0x%08x | locked=%d owner=%u depth=%u rec=%d",
              self, addr, c.lr(), (int)gm->locked, gm->owner, gm->depth, (int)gm->recursive);
...
void c_create(GuestCall &c) {
    ...
    // Bypass Transmension telemetry LogManager::threadSendProc thread
    if ((start_routine & ~1u) == (c.img->modules[0].base + 0x004faab8)) {
...
void c_attr_getstack(GuestCall &c) {
    if (c.arg(1) != 0) {
        uint32_t sp = c.sp();
        uint32_t stack_base = sp & ~0x000FFFFFu;
        c.write32(c.arg(1), stack_base);
    }
